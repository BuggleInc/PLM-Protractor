describe('Exo15', function() {
    ...
    beforeEach(function() {
        browser.get('https://plm.telecomnancy.univ-lorraine.fr/#/ui/lessons/welcome/welcome.lessons.welcome.loopfor.LoopCourseForest');
    ...
    it('should congrats the user for passing the exercise', function() {
        browser.executeScript("window.editor.setValue(\"public void run() {for (int i = 0; i<7;i++) for (int side=0;side<4;side++){for (int step=0;step<4;step++)forward();left();for (int step=0;step<2;step++)forward();right();for (int	step=0;step<4;step++)forward();right();forward();forward();left();for (int step=0;step<4;step++)forward();left();}}\"); ");
        browser.wait(until.visibilityOf(button), 500, "Button unclickable");
        button.click();
