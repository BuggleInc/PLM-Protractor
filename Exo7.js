describe('Exo7', function() {
    ...
    beforeEach(function() {
        browser.get('https://plm.telecomnancy.univ-lorraine.fr/#/ui/lessons/welcome/welcome.lessons.welcome.variables.Variables');
    ...
    it('should congrats the user for passing the exercise', function() {
        browser.executeScript("window.editor.setValue(\"public void run() {int cpt = 0;while (!isOverBaggle()) {cpt++;forward();}pickupBaggle();while (cpt>0) {backward();cpt--;}dropBaggle();}\"); ");
        browser.wait(until.visibilityOf(button), 500, "Button unclickable");
        button.click();
